...
def crawl_data_from_wencai(question: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            cols = pd.Series([re.sub(r'\[[^)]*\]', '', col)
                             for col in pd.Series(df_data.columns)])
...
def renderHtml(df, filename: str, title: str):
    ...
    with open(filename, 'w') as f:
        f.write(html_string.replace('<table border="1" class="dataframe">',
                '<table id="container">').replace('<th>', '<th role="columnheader">'))
...
class Bot():
    ...
    def chatgpt(self, queryText: str):
        ...
        if self.chatgptBot is None:
            self.chatgptBot = ChatGPT(
                config={"access_token": os.environ['CHATGPT'], 'proxy': PROXY})
...
if __name__ == '__main__':
    ...
    for k, v in wdf.iterrows():
        symbol = k.split('.')[0]
        wdf.at[k, 'stock'] = '<a href="https://xueqiu.com/S/%s">%s%s</a>' % (
            k[-2:]+symbol, k[-2:]+symbol, v['股票简称'])
        news = ak.stock_news_em(symbol)
        news.drop_duplicates(subset='新闻标题', inplace=True)
        news['发布时间'] = pd.to_datetime(news['发布时间'])
        news['新闻标题'] = news['发布时间'].dt.strftime(
            '%Y-%m-%d ')+news['新闻标题'].str.replace('%s：' % v['股票简称'], '')
        news = news[~news['新闻标题'].str.contains('股|主力|机构|资金流')]
        news['news'] = news['新闻标题'].str.cat(
            news['新闻内容'].str.split('。').str[0], sep=' ')
        ...
        while retry > 0:
            try:
                ...
                if isinstance(parsed['风险'], list):
                    ...
                    risks = parsed['风险']
                wdf.at[k, 'chance'] = chances.replace(
                    v['股票简称'], '').replace('\n', '<br>')
                wdf.at[k, 'risk'] = risks.replace(
                    v['股票简称'], '').replace('\n', '<br>')
                if '\n' in risks:
